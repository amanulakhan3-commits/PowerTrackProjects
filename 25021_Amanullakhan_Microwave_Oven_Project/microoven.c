...
unsigned char sec,min,pre_heat_time;//global variable
unsigned char operational_flag; 


void power_on_screen(void)  // power on welcome screen
{
    for (unsigned int i=0;i<16;i++) // Display top loading bar
    {
...
void set_time(unsigned char key,unsigned char reset_flag)
{
    ...
    if(reset_flag==RESET_MODE) // reseting the values while entering screen
    {
        blink_pos=0;
        key_count=0;
        key=ALL_RELEASED;
...
//the KEY * for clearing the entered value
    else if(key=='*')
    {
        if(blink_pos)
        {
...
//the KEY  #  is used to start microwave oven 
    else if(key=='#')
    {
        //turn on the fan
        FAN=1;
...
void time_display(void)
{
    ...
    if(min==0 && sec==0)
    {
        ...
        //display the message after  cooking completed
        TMR2ON=0;
        FAN=0;
...
void set_temp(unsigned char key,unsigned char reset_flag)
{
    ...
    //the matrix keypad keys 0 to 9 are used to enter temperature
     ...
     else if(key=='#')
     {
         ...
         //wait until pre heating contiues
